    ...
    PaymentOptions options) : IEscrowProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var paymentStatus = eventName == "charge.success" || status == "success"
            ? PaymentStatus.Paid
        ...
        return Result<PaymentNotification>.Ok(
            new PaymentNotification(reference, paymentStatus, paidMinor));
    }
    ...
    private sealed record TransferData(string Reference, string? TransferCode);
}